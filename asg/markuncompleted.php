...
<form align="center" name="form" action="markuncompleted.php" onsubmit="return validate()" method="POST">
	<?php
$connection=mysqli_connect("localhost", "root", "", "posters");
if(isset($_REQUEST['submitted']))
{
	$id=$_REQUEST['id'];
	$query=mysqli_query($connection,"SELECT `id` FROM `clients` WHERE `id`='$id'");
if(mysqli_num_rows($query)>0){
	mysqli_query($connection,"UPDATE `clients` SET `completed`='0' WHERE `id`='$id'");
	echo "id no ".$id." marked as un completed";
	}
else
	{
	echo "invalid id";	
